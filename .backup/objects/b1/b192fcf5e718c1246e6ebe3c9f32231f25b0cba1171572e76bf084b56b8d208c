...
class HapticEngine(
    ...
    // v4.13: RichTap 硬件抽象层 - 小米/Redmi设备直调私有API
    private val richTapAdapter = RichTapAdapter()
    @Volatile private var richTapAvailable = false
    @Volatile private var hapticMode = "balanced"  // 震感模式: balanced/bass/crisp/soft/immersive/pure
    @Volatile private var masterIntensity = 1.0f  // 主强度 0.0-2.0
    ...
    init {
        ...
        // v4.13: 检测 RichTap 可用性
        richTapAvailable = RichTapAdapter.isAvailable()
        Log.i(TAG, "[RichTap] available=$richTapAvailable on ${Build.MANUFACTURER} ${Build.MODEL}")
        LogBroadcaster.sendLog(context, "[RichTap] available=$richTapAvailable")
     ...
     * 
     * v4.13: RichTap 优先路径 - 小米/Redmi设备直调私有API，绕过标准Vibrator延迟
     */
    private fun triggerBeatVibration(event: String, intensity: Int) {
        ...
        // v4.13: RichTap 优先路径
        if (richTapAvailable) {
            triggerRichTapVibration(event, intensity)
            lastBeatEvent = event
            return
        }
    ...
    /**
     * v4.13: RichTap 直接驱动路径
     * 
     * 参考 14PRO_MusicHaptic 模块的实现，使用 MIUI/HyperOS 私有 API：
     * DynamicEffect.create() → createContinuous(amplitude, sharpness, duration) → HapticPlayer.start()
     * 
     * 优势：
     * 1. 绕过 Android 标准 Vibrator 延迟（约 10-20ms → 约 2-5ms）
     * 2. 直接控制包络曲线（attack-decay）
     * 3. 硬件级限频保护
     */
    private fun triggerRichTapVibration(event: String, intensity: Int) {
        val now = SystemClock.elapsedRealtime()
        val normalized = (intensity.coerceIn(0, 255) / 255f).coerceIn(0.05f, 1.0f)
        
        // 基于模式的最小间隔（限频保护）
        val minInterval = RichTapAdapter.getMinIntervalMs(hapticMode)
        
        // 全局 refractory 检查
        val lastVib = lastVibrationMs.get()
        if (lastVib > 0 && now - lastVib < minInterval) {
            // 静默丢弃 - 不打日志避免刷屏
            return
        }
        lastVibrationMs.set(now)

        // 根据节拍类型映射 sharpness 和 duration
        // KICK=低sharpness(柔和), SNARE=中sharpness, TICK=高sharpness(短促)
        val (sharpness, duration) = when (event.uppercase()) {
            "KICK"  -> Pair(0.32f, 0.058f)   // 柔和冲击
            "SNARE" -> Pair(0.48f, 0.045f)   // 中等冲击
            "TICK"  -> Pair(0.66f, 0.030f)   // 短促尖锐
            else    -> Pair(0.40f, 0.050f)   // 默认
        }

        // 应用模式和强度设置
        val modeSharpnessMod = when (hapticMode) {
            "bass" -> sharpness - 0.07f      // 更柔和
            "crisp" -> sharpness + 0.12f     // 更尖锐
            "soft" -> sharpness - 0.05f      // 柔和
            "immersive" -> sharpness          // 不变
            "pure" -> sharpness * 0.8f        // 极简
            else -> sharpness                  // balanced
        }.coerceIn(0.1f, 0.9f)

        val amplitude = (normalized * masterIntensity).coerceIn(0.05f, 1.0f)
        val attack = 0.002f  // 2ms attack

        try {
            RichTapAdapter.trigger(amplitude, modeSharpnessMod, duration, attack)
            
            if (now - lastVibrationMs.get() > 3000 || intensity > 200) {
                val msg = "[RichTap] $event amp=${"%.2f".format(amplitude)} sharp=${"%.2f".format(modeSharpnessMod)} dur=${duration}s mode=$hapticMode"
                Log.i(TAG, msg)
                LogBroadcaster.sendLog(context, msg)
            }
        } catch (e: Exception) {
    ...
    fun synchronizeParameters() {
        ...
        // v4.13: 同步 RichTap 模式和强度设置
        hapticMode = try { prefs.getString("haptic_mode", "balanced") ?: "balanced" } catch (e: Exception) { "balanced" }
        masterIntensity = try { prefs.getFloat("master_intensity", 1.0f) } catch (e: Exception) { 1.0f }
        Log.i(TAG, "[Settings-SYNC] mode=$hapticMode masterIntensity=${"%.2f".format(masterIntensity)} richTap=$richTapAvailable")
    }
