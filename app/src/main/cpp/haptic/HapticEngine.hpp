...
public:
    ...
    void composeHapticLayer(float subRms, float midRms, float textureRms,
        ...
        // v3.8.6 FIX: Lowered threshold from 0.015 to 0.003 — quiet music (rms~0.009)
        // was being zeroed out entirely, killing the whole vibration chain.
        float totalEnergy = kickEnv + snareEnv + vocalEnv + bodyEnv;
        if (totalEnergy < 0.003f) {
