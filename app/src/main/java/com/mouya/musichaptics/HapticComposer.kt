...
class HapticComposer(
    ...
    fun processFrame(
        ...
        // v3.11: Generate beat-first haptics (Beat tap + HiHat) — no floor
    ...
    // ════════════════════════════════════════════════════════════════
    //  v3.10: iOS Music Haptics — Multi-layer haptic generation
    //  Mirrors Apple's 4-layer approach: Beats + Bass + Vibrations + Hits
    // ════════════════════════════════════════════════════════════════
    ...
    /**
     * v3.10: Generate additional haptic layers inspired by iOS Music Haptics.
     *
     * Apple's system layers four independent haptic tracks:
     * 1. Beats — crisp transient taps on every beat (hapticTransient)
     * 2. Bass — continuous low-frequency body vibration (hapticContinuous)
     * 3. Vibrations — melodic/vocal sustained waves (hapticContinuous)
     * 4. Hits — sharp impacts for drum hits (hapticTransient)
     *
     * We generate the first three here as additionalPrimitives, while the
     * primary primitive (from mapEventToPrimitive) covers the "Hits" layer.
     */
    private fun generateIosStyleLayers(
        ...
        // ════════════════════════════════════════════════════════════════
        //  v3.11: Beat-first haptic strategy (inspired by NetEase 嗨动模式)
        //
        //  Philosophy: transient > sustained. No continuous "floor" that
        //  masks beat taps. Every vibration should be a decisive pulse.
        //  Amplitude follows music energy — quiet sections are gentle,
        //  climaxes hit hard.
        // ════════════════════════════════════════════════════════════════

        // ── BEAT TAP (core layer, like NetEase 节拍震动) ──
        // Every detected beat → one crisp transient. No sustained floor.
        // Refractory: 120ms (supports up to 500 BPM, most songs 80-180 BPM)
        if (isBeat && timestamp - lastBeatTapMs >= 120L) {
            lastBeatTapMs = timestamp

            // Beat strength: driven by actual audio energy, NOT a fixed floor.
            // Quiet section → weak tap (~30%), climax → full punch (100%)
            // No +0.2f floor anymore — let quiet be quiet.
            ...
            // Strong beat (kick detected on this beat) vs weak beat
            val isStrongBeat = isKeyStrike || instruments.kick > 0.3f
            val beatIntensity = if (isStrongBeat) {
                (beatGamma * 255f * persona.impactBias).toInt().coerceIn(120, 255)
            } else {
                (beatGamma * 180f * persona.impactBias).toInt().coerceIn(30, 200)
            }

            // Short, crisp transient — like NetEase's decisive beat pulse
            layers.add(HapticPrimitive.Impact(
                intensity = beatIntensity,
                durationMs = 20,
                velocityFactor = beatStrength,
                sharpness = if (isStrongBeat) 0.7f else 0.5f,
                semantic = if (isStrongBeat) "BEAT_TAP_STRONG" else "BEAT_TAP"
            ))
        }

        // ── HI-HAT TEXTURE (light, non-floor) ──
        // Very short ticks that don't create sustained vibration.
        // Only when hi-hat confidence is high, and spaced out enough.
        if (instruments.hiHat > 0.40f && timestamp - lastInstrumentEventMs >= 60L) {
            val hatIntensity = (instruments.hiHat * 100f * persona.textureBias).toInt().coerceIn(25, 120)
        ...
        // NOTE: BASS_BODY and VOCAL_WAVE layers have been REMOVED in v3.11.
        // They created a continuous "floor" that masked the beat taps,
        // making the experience muddy instead of crisp and decisive.
        // The dynamic gain (computeDynamicGain) already provides energy-
        // following amplitude modulation — we don't need a sustained bed.
...
data class HapticCommand(
    ...
    // v3.10: iOS-style multi-layer haptics — additional simultaneous primitives
    // (Beat tap, Bass body, Vocal wave) that play alongside the primary primitive.
